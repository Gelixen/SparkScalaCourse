...
object DegreesOfSeparationDataset {
  ...
  def exploreNode(spark: SparkSession, ds: Dataset[BFSNode], iteration: Int): (Dataset[BFSNode], Long) = {
    ...
    val filteredRawExploreDS = rawExploreDS
      .select($"id", explode($"connections").alias("child"))
      .distinct()

    rawExploreDS.show()
    filteredRawExploreDS.show()

    val hitCount = filteredRawExploreDS.filter($"child" === targetCharacterID).count()
    val exploreDS = filteredRawExploreDS.distinct().select("child")
